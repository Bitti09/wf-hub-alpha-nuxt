  ...
  methods: {
    ...
    formatTimer(diff) {
      ...
      ;[
        ...
        if (!first || time > 0) {
          stringArray.push(
            time.toString().padStart(first ? 1 : 2, '0') + suffix
          )
        }
        timeLeft -= time * unit
      })
    ...
    onBadgeUpdate() {
      ...
      if (!this.counter) {
        ...
        if (!this.endtime) {
          this.disp = `${diffactivate > 0 ? '-' : ''}${this.formatTimer(
            Math.abs(diffactivate)
          )}`
        } else if (typeof diffactivate !== 'undefined' && diffactivate > 0) {
          this.mutableVariant = 'info'
          this.disp = `${this.$t('time.startL')} ${this.formatDurationShort(
            durationactivate
          )}`
        } else if (diff < 0) {
          this.mutableVariant = 'info'
          this.disp = `${this.$t('time.expiredL')}: ${this.formatDurationShort(
            duration
          )}`
        } else {
          if (diff < 600000) {
        ...
        if (typeof diffactivate !== 'undefined' && diffactivate > 0) {
          ...
          this.mutableVariant = 'transparent'
          this.disp = `${this.$t('time.ongoingL')} ${this.formatDurationShort(
            duration
          )}`
        }
      }
